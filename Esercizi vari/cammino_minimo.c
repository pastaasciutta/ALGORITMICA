...
void bfs(edges *E, int n, int from,int v2);
...
int starting_node,finish_node, m=0;

// leggo il numero di query
scanf("%d", &m);

for(short i=0;i<m;i++){
  // leggo il nodo da cui partire
  scanf("%d", &(starting_node));
  // leggo il nodo a cui arrivare
  scanf("%d", &(finish_node));
bfs(edges, num_nodes, starting_node,finish_node);
}
